...
par.parcelid IN (SELECT parcelid FROM invt.no_utilities_2013_by_parcel WHERE EXTRACT(year FROM last_bill) < 2011) AS no_utilities,
...
fc.parcels, fc.f08 as fc4, fc.f09 as fc3, fc.f10 as fc2, fc.f11 as fc1,
...
LEFT JOIN parcels.par11_bg_lookup bglook ON (bglook.parid=par.parcelid)
LEFT JOIN parcels.par11_blk_lookup blklook ON (blklook.parid=par.parcelid)
...
LEFT JOIN (SELECT parid, EXTRACT (year FROM saledt) yrsold, saletype stype, saleval sval, steb, price,
           ...
           FROM tax2011_sales) sal ON (sal.parid = par.parcelid AND sal.rn = 1)    

-- aggregate of recent permits
LEFT JOIN (SELECT parid, SUM(amount) AS total FROM tax2011_permit WHERE EXTRACT(YEAR FROM permdt) >= 2011 - 10 GROUP BY parid) prmt ON (prmt.parid = par.parcelid)

-- aggregate of outbuildings
LEFT JOIN (SELECT parid, ARRAY_AGG(code) AS oby_code, ARRAY_AGG(yrblt) AS oby_yrblt FROM tax2011_oby GROUP BY parid) oby ON (oby.parid = par.parcelid)
...
LEFT JOIN (SELECT tractid, sum(gross_sales) as gross_sales from sales.tax where date<'01-01-2011' and date>'12-31-2008' group by tractid) stax ON (par.tractid=stax.tractid)
...
LEFT JOIN (SELECT sum((we_type='W')::int) as weeds, sum((we_type='I')::int) as sidewalks, sum((we_type='A')::int) as sanitation, sum((we_type='N')::int) as neglect,
		...
		WHERE cast(we_fullyear as int)<=2011 and cast(we_fullyear as int)>=2008 GROUP BY we_mapno) cca on (par.parcelid=cca.we_mapno)
LEFT JOIN (SELECT sum(tb_latefee) as tb_latefee, tb_mapno, sum((length(tb_bankrupt)>0)::int) as tb_bankrupt, sum((length(tb_badcheck)>0)::int) as tb_badcheck 
		FROM com.com_tax_delinquent WHERE cast(tb_fullyear as int)<=2011 and cast(tb_fullyear as int)>=2008 GROUP BY tb_mapno) ctd on (par.parcelid=ctd.tb_mapno)
