SELECT par.parcelid, ST_X(ST_TRANSFORM(par.centroid,4326)) as lon, ST_Y(ST_TRANSFORM(par.centroid,4326)) as lat, fray.condgrade,
...
par.parcelid IN (SELECT parcelid FROM invt.no_utilities_2013_by_parcel WHERE EXTRACT(year FROM last_bill) < 2013) AS no_utilities,
...
fc.parcels, fc.f10 as fc4, fc.f11 as fc3, fc.f12 as fc2, fc.f13 as fc1,
...
FROM parcels.parcels2013 par
LEFT JOIN frayser.exportdata fray ON (fray.parcelid=par.parcelid)
LEFT JOIN parcels.par13_bg_lookup bglook ON (bglook.parid=par.parcelid)
LEFT JOIN parcels.par13_blk_lookup blklook ON (blklook.parid=par.parcelid)

LEFT JOIN tax2013_asmt asmt  ON (asmt.parid = par.parcelid)
LEFT JOIN tax2013_owndat od  ON (od.parid = par.parcelid)
LEFT JOIN tax2013_pardat pd  ON (pd.parid = par.parcelid)

LEFT JOIN tax2009_asmt asmt4 ON (asmt4.parid = par.parcelid)
LEFT JOIN (SELECT *, ROW_NUMBER() OVER (PARTITION BY parid ORDER BY sfla DESC) rn FROM tax2009_dweldat) dd4 ON (dd4.parid = par.parcelid AND dd4.rn=1)

-- this grabs dweldat for only the largest property
LEFT JOIN (SELECT *, ROW_NUMBER() OVER (PARTITION BY parid ORDER BY sfla DESC) rn FROM tax2013_dweldat) dd ON (dd.parid = par.parcelid AND dd.rn=1)
...
LEFT JOIN (SELECT parid, EXTRACT (year FROM saledt) yrsold, saletype stype, saleval sval, steb, price,
           ...
           FROM tax2013_sales) sal ON (sal.parid = par.parcelid AND sal.rn = 1)    

-- aggregate of recent permits
LEFT JOIN (SELECT parid, SUM(amount) AS total FROM tax2013_permit WHERE EXTRACT(YEAR FROM permdt) >= 2013 - 10 GROUP BY parid) prmt ON (prmt.parid = par.parcelid)

-- aggregate of outbuildings
LEFT JOIN (SELECT parid, ARRAY_AGG(code) AS oby_code, ARRAY_AGG(yrblt) AS oby_yrblt FROM tax2013_oby GROUP BY parid) oby ON (oby.parid = par.parcelid)
...
LEFT JOIN (SELECT tractid, sum(gross_sales) as gross_sales from sales.tax where date<'01-01-2013' and date>'12-31-2010' group by tractid) stax ON (par.tractid=stax.tractid)
...
LEFT JOIN (SELECT sum((we_type='W')::int) as weeds, sum((we_type='I')::int) as sidewalks, sum((we_type='A')::int) as sanitation, sum((we_type='N')::int) as neglect,
		...
		WHERE cast(we_fullyear as int)<=2013 and cast(we_fullyear as int)>=2010 GROUP BY we_mapno) cca on (par.parcelid=cca.we_mapno)
LEFT JOIN (SELECT sum(tb_latefee) as tb_latefee, tb_mapno, sum((length(tb_bankrupt)>0)::int) as tb_bankrupt, sum((length(tb_badcheck)>0)::int) as tb_badcheck 
		FROM com.com_tax_delinquent WHERE cast(tb_fullyear as int)<=2013 and cast(tb_fullyear as int)>=2010 GROUP BY tb_mapno) ctd on (par.parcelid=ctd.tb_mapno)
