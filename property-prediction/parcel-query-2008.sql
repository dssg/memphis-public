SELECT par.parcelid, ST_X(ST_TRANSFORM(par.centroid,4326)) as lon, ST_Y(ST_TRANSFORM(par.centroid,4326)) as lat, aud.strprob, 
...
fc.parcels, fc.f05 as fc4, fc.f06 as fc3, fc.f07 as fc2, fc.f08 as fc1,
...
LEFT JOIN nbn.final aud USING (parcelid)
LEFT JOIN parcels.par08_bg_lookup bglook ON (bglook.parid=par.parcelid)
LEFT JOIN parcels.par08_blk_lookup blklook ON (blklook.parid=par.parcelid)

LEFT JOIN tax2008_asmt asmt  ON (asmt.parid = par.parcelid)
LEFT JOIN tax2008_owndat od  ON (od.parid = par.parcelid)
LEFT JOIN tax2008_pardat pd  ON (pd.parid = par.parcelid)

LEFT JOIN tax2004_asmt asmt4 ON (asmt4.parid = par.parcelid)
LEFT JOIN (SELECT *, ROW_NUMBER() OVER (PARTITION BY parid ORDER BY sfla DESC) rn FROM tax2004_dweldat) dd4 ON (dd4.parid = par.parcelid AND dd4.rn=1)

-- this grabs dweldat for only the largest property
LEFT JOIN (SELECT *, ROW_NUMBER() OVER (PARTITION BY parid ORDER BY sfla DESC) rn FROM tax2008_dweldat) dd ON (dd.parid = par.parcelid AND dd.rn=1)
...
LEFT JOIN (SELECT parid, EXTRACT (year FROM saledt) yrsold, saletype stype, saleval sval, steb, price,
           ...
           FROM tax2008_sales) sal ON (sal.parid = par.parcelid AND sal.rn = 1)    
...
LEFT JOIN (SELECT tractid, sum(gross_sales) as gross_sales from sales.tax where date<'01-01-2009' and date>'12-31-2006' group by tractid) stax ON (par.tractid=stax.tractid)
...
LEFT JOIN (SELECT sum((we_type='W')::int) as weeds, sum((we_type='I')::int) as sidewalks, sum((we_type='A')::int) as sanitation, sum((we_type='N')::int) as neglect,
		...
		WHERE cast(we_fullyear as int)<=2008 and cast(we_fullyear as int)>=2005 GROUP BY we_mapno) cca on (par.parcelid=cca.we_mapno)
LEFT JOIN (SELECT sum(tb_latefee) as tb_latefee, tb_mapno, sum((length(tb_bankrupt)>0)::int) as tb_bankrupt, sum((length(tb_badcheck)>0)::int) as tb_badcheck 
		FROM com.com_tax_delinquent WHERE cast(tb_fullyear as int)<=2008 and cast(tb_fullyear as int)>=2005 GROUP BY tb_mapno) ctd on (par.parcelid=ctd.tb_mapno)
